...
## 1. Create the new OAuth Client (same project, skip API/consent-screen setup)
...
`drive.file` — it can only see/write files it creates or that are explicitly shared with it, not
your whole Drive). The refresh token is stored in Keychain as
`tax-agent-drive-oauth-refresh-token` — a new, separate entry from the sibling tool's
`vendsoft-drive-oauth-refresh-token`.
...
After steps 1–2 are done, the upload step in `src/sync.py` reads the refresh token from Keychain
